...
void deck__shuffle(Deck *deck)
{
    ...
    while (!queue__is_empty(deck) || !queue__is_empty(support))
    { //FACCIO IL MERGE DEI DUE MAZZETTI ALTERNANDO LE CARTE A RANDOM
        if ((rand() % 2 || queue__is_empty(support)) && !queue__is_empty(deck))
